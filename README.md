...
本分支在 1.4.2 基础上补充注册与生图链路稳定性能力，核心改动如下：

- 新增默认关闭的稳定代理运行时 `proxy_runtime`。
- 引入 WARP + Privoxy + FlareSolverr 组合部署。
- 注册遇到 Cloudflare 拦截后刷新 clearance 并重试。
- 增加 FlareSolverr clearance 缓存、测试接口与脱敏展示。
- 后台设置页新增「稳定代理运行时」配置面板。
- 新增 `docker-compose.warp.yml` 与幂等初始化脚本。
- 代理优先级：账号代理 > 稳定代理运行时 > 显式代理 > 全局代理。
- 补充代理运行时、注册重试和初始化脚本单元测试。
